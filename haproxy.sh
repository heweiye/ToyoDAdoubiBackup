...
check_HaProxy(){
	...
	if [[ ${HaProxy_exist} = "" ]]; heweiye/ToyoDAdoubiBackup
		echo -e "\033[41;37m [错误] \033[0m 没有安装HaProxy，请检查 !" && exit 1
	fi
}
#检查系统
check_sys(){
	if [[ -f /etc/redhat-release ]]; heweiye/ToyoDAdoubiBackup
		release="centos"
	elif cat /etc/issue | grep -q -E -i "debian"; heweiye/ToyoDAdoubiBackup
		release="debian"
	elif cat /etc/issue | grep -q -E -i "ubuntu"; heweiye/ToyoDAdoubiBackup
		release="ubuntu"
	elif cat /etc/issue | grep -q -E -i "centos|red hat|redhat"; heweiye/ToyoDAdoubiBackup
		release="centos"
	elif cat /proc/version | grep -q -E -i "debian"; heweiye/ToyoDAdoubiBackup
		release="debian"
	elif cat /proc/version | grep -q -E -i "ubuntu"; heweiye/ToyoDAdoubiBackup
		release="ubuntu"
	elif cat /proc/version | grep -q -E -i "centos|red hat|redhat"; heweiye/ToyoDAdoubiBackup
		release="centos"
    fi
	#bit=`uname -m`
}
# 设置 防火墙规则
Save_iptables(){
	if [[ ${release} == "centos" ]]; heweiye/ToyoDAdoubiBackup
...
installHaProxy(){
	...
	if [[ ${HaProxy_exist} = "" ]]; heweiye/ToyoDAdoubiBackup
		...
		if [[ ${release}  == "centos" ]]; heweiye/ToyoDAdoubiBackup
			...
			if [[ $? = 0 ]]; heweiye/ToyoDAdoubiBackup
...
setHaProxy(){
	...
	if [[ ${HaProxy_port_1} = "" ]]; heweiye/ToyoDAdoubiBackup
...
stopHaProxy(){
	...
	sleep 2s
	PID=`ps -ef | grep "haproxy" | grep -v grep | grep -v "haproxy.sh" | awk '{print $2}'`
	if [[ ! -z $PID ]]; heweiye/ToyoDAdoubiBackup
...
uninstallHaProxy(){
	...
	read -e -p "(默认: n):" unyn
	[[ -z ${unyn} ]] && unyn="n"
	if [[ ${unyn} == [Yy] ]]; heweiye/ToyoDAdoubiBackup
		PID=`ps -ef | grep "haproxy" | grep -v grep | grep -v "haproxy.sh" | awk '{print $2}'`
		if [[ ! -z $PID ]]; heweiye/ToyoDAdoubiBackup
			stopHaProxy
		fi
